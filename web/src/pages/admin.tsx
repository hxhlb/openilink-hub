...
import { Save, Trash2, Blocks, X } from "lucide-react";
...
function AdminAppsTab() {
  ...
  async function handleDelete(app: any) {
    if (!confirm(`删除 App "${app.name}"？此操作不可恢复，所有安装实例也会被删除。`)) return;
    try { await api.deleteApp(app.id); setSelected(null); load(); } catch (err: any) { setError(err.message); }
  }

  const tools = (selected?.tools || []) as any[];
  const events = (selected?.events || []) as string[];
  const scopes = (selected?.scopes || []) as string[];
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-1">
        {apps.map((app) => (
          <div key={app.id} onClick={() => setSelected(selected?.id === app.id ? null : app)}
          ...
          <div className="bg-background border rounded-xl max-w-lg w-full mx-4 max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="p-4 border-b flex items-start justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">{selected.description || "无描述"}</p>
                <div className="flex gap-3 mt-1 text-xs text-muted-foreground">
                  {selected.owner_name && <span>拥有者: {selected.owner_name}</span>}
                  {selected.homepage && <a href={selected.homepage} target="_blank" rel="noopener" className="text-primary hover:underline">主页</a>}
                  <span>{new Date(selected.created_at * 1000).toLocaleDateString()}</span>
                </div>
              </div>
              <button onClick={() => setSelected(null)} className="text-muted-foreground hover:text-foreground cursor-pointer">
                <X className="w-4 h-4" />
              </button>
            </div>
